<?php
/*
Plugin Name: Recipe SEO Nutrition and Print by Edamam 
Plugin URI:http://wordpress.org/plugins/seo-nutrition-and-print-for-recipes-by-edamam/
...
if (!defined('EDAMAM_RECIPE_VERSION_NUM'))
    define('EDAMAM_RECIPE_VERSION_NUM', '1.0'); //!!mwp
...
add_action('media_buttons', 'edamam_recipe_add_recipe_button', 30);
add_action('init', 'edamam_recipe_enhance_mce');
...
function edamam_recipe_enhance_mce() {
...
function edamam_recipe_tinymce_plugin($plugin_array) {
   ...
   $plugin_array['edamamrecipe'] =  get_option('siteurl') . '/wp-content/plugins/' . dirname(plugin_basename(__FILE__)) . '/editor.js';
...
function edamam_recipe_add_recipe_button() {
	...
	$media_edamam_recipe_iframe_src = apply_filters('media_edamam_recipe_iframe_src', "$media_upload_iframe_src&amp;type=edamam_recipe&amp;tab=edamam_recipe");
	$media_edamam_recipe_title = __('Add a Recipe', 'wp-media-edamam_recipe');

	echo "<a class=\"thickbox\" href=\"{$media_edamam_recipe_iframe_src}&amp;TB_iframe=true&amp;height=500&amp;width=640\" title=\"$media_edamam_recipe_title\"><img src='" . get_option('siteurl').'/wp-content/plugins/'.dirname(plugin_basename(__FILE__)) . "/edamam.gif?ver=1.0' alt='EDRecipe Icon' /></a>";
}
...
function edamam_recipe_iframe_content($post_info = null, $get_info = null) {
    ...
    if ($post_info || $get_info) {
        ...
        if ($get_info["post_id"] && !$get_info["add-recipe-button"] && strpos($get_info["post_id"], '-') !== false) {
            ...
            //!!mwp $ingredients_list = edamam_recipe_select_ingredients_db($recipe_id);
